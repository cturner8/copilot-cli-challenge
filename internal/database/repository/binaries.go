...
format, asset_regex, release_regex, config_digest, created_at, updated_at, config_version, source)
VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
`, binary.UserID, binary.Name, binary.Alias, binary.Provider, binary.ProviderPath,
		...
		binary.ConfigDigest, binary.CreatedAt, binary.UpdatedAt, binary.ConfigVersion, binary.Source)
...
config_digest = ?, updated_at = ?, config_version = ?, source = ?
WHERE id = ?
`, binary.UserID, binary.Name, binary.Alias, binary.Provider, binary.ProviderPath,
		binary.InstallPath, binary.Format, binary.AssetRegex, binary.ReleaseRegex,
		binary.ConfigDigest, binary.UpdatedAt, binary.ConfigVersion, binary.Source, binary.ID)
...
`, id).Scan(&binary.ID, &binary.UserID, &binary.Name, &binary.Alias, &binary.Provider,
		...
		&binary.ReleaseRegex, &binary.ConfigDigest, &binary.CreatedAt, &binary.UpdatedAt, &binary.ConfigVersion, &binary.Source)
...
`)
	...
	for rows.Next() {
		...
		err := rows.Scan(&binary.ID, &binary.UserID, &binary.Name, &binary.Alias,
			...
			&binary.ConfigVersion, &binary.Source)
		if err != nil {
...
func (r *BinariesRepository) ListWithVersionDetails(noActiveVersionLabel string) ([]*BinaryWithVersionDetails, error) {
	query := `
		SELECT 
			...
			b.format, b.asset_regex, b.release_regex, b.config_digest, b.created_at, b.updated_at, b.config_version, b.source,
	...
	for rows.Next() {
		...
		err := rows.Scan(
			...
			&binary.Provider, &binary.ProviderPath, &binary.InstallPath, &binary.Format,
			&binary.AssetRegex, &binary.ReleaseRegex, &binary.ConfigDigest, &binary.CreatedAt, &binary.UpdatedAt,
			&binary.ConfigVersion, &binary.Source, &activeVersion, &details.InstallCount,
